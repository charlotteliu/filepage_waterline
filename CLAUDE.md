...
3. **Analyze** (`outputs/cold_page_stats.py`): non-visual stats tool. Streams add/delete/access(+`label`>1) events in timestamp order, reconstructs each `(dev,ino,ofs)` add→delete interval, flags **cold pages** (≤1 access), and reports cold **spacetime** (page-seconds; 0-access = delete−add, 1-access = delete−access) and average cold footprint (spacetime ÷ test window). Same streaming/index/pragma performance model as the visualizer; `--build-indices`, `--no-label-access`, `--top`, `--json`, `--csv`, and `--exclude-file GLOB` (mirrors the visualizer: repeatable shell-glob drop of `inode_mapping.filename` matches, resolved once to `dev|ino` keys via `resolve_excluded_file_keys` and applied as a membership test in `iter_events`).
