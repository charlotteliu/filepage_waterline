...
3. **Analyze** (`outputs/cold_page_stats.py`): non-visual stats tool. Streams add/delete/access(+`label`>1) events in timestamp order, reconstructs each `(dev,ino,ofs)` add→delete interval, flags **cold pages** (≤1 access), and reports cold **spacetime** (page-seconds; 0-access = delete−add, 1-access = delete−access) and average cold footprint (spacetime ÷ test window). Also reports **average resident space** (`averageResidentSpace`, pages/bytes/MB) — the same spacetime÷test-window average but over *every* closed interval's full `delete−add` duration, not just cold ones — and cold's share of it (`averageColdSpace.fractionOfAverageResident`). Same streaming/index/pragma performance model as the visualizer; `--build-indices`, `--no-label-access`, `--top`, `--json`, `--csv`.
