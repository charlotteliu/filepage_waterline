...
5. **Analyze** (`outputs/single_process_page_stats.py`): non-visual stats tool. Computes the deduped **population** of `(dev,ino,ofs)` pages seen in `mm_filemap_add_to_page_cache` ("loaded") ∪ `bitmap_page_info` (mmap-observed), then reports how many of those are **single-owner and unmapped**: `mmapcnt==0` across all their add/delete/access rows *and* touched by exactly one `pid_name` (add + delete + access-history + the bitmap snapshot's own pid), plus that count's % of the population. A page seen only in bitmap (no add/delete/access rows at all) has no mmapcnt data and is excluded from the mmapcnt==0 cohort rather than defaulted. Four independent full-table scans (no timestamp-merge needed, no per-key state machine); `--no-access`, `--no-bitmap-pid`, `--build-indices`, `--top`, `--json`, `--csv`.
